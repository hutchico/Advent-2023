...
public class Main {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		HashMap<Character, Integer> deck = new HashMap<>();
		deck.put('A',0);
		deck.put('K',1);
		deck.put('Q', 2);
		deck.put('T', 3);
		...
		while(inp.hasNext()) {
			...
			Hands.add(new Hand(raw[0],Integer.valueOf(raw[1]),-1,-1));
		}
		
		//part 1: iterate through Hands + evaluate wins, assign basic rank to each
		for(int i = 0; i < Hands.size(); i++) {
			Hand one = Hands.get(i);
			//System.out.println(one.get_key());
			HashMap<Character,Integer> nu = new HashMap<Character,Integer>();
			for(int n = 0; n < 5; n++) {
				char card = ((String) one.get_key()).charAt(n);
				if (nu.get(card) != null) 
					nu.put(card, nu.get(card) + 1);
				else
					nu.put(card, 1);
			}
			//System.out.println("dict size: " + nu.size());
			int jcount;
			if(nu.get('J') != null) {
				jcount = nu.get('J');
				nu.remove('J');
				...
				for(Map.Entry<Character, Integer> ea : nu.entrySet()) {
				...
				if(counts.size() == 0) {
					...
					int val = -1;
					char fchoice = find_char(one.get_key(),deck); //find the highest card suite in hand
					char cchoice = counts.get(0).get_key();
					char choice = ' ';
					if(cchoice == 'J')
						cchoice = counts.get(1).get_key();
					if(deck.get(cchoice) > deck.get(fchoice)) {
						choice = cchoice;
						val = nu.get(cchoice);
					}
					else {
			...
			case(3): //either three kind or two pair
				...
				int which = 0;
				for(int ea : nu.values()) { //hacky but saves using another arraylist
					counts[which] = ea; 
					which++;
				}
		...
		int sum = 0;
		for(int i = 0; i < Hands.size(); i++) { //oh it's weakest to strongest
			sum += Hands.get(i).get_value() * (i + 1);
		}
